...
<li> Subtopics for exploration instead of prescriptive steps,
...
good search query, and add it after a slash ("/") after the subtopic.  Try
Googling a few search queries that you think are reasonable for that topic and
capture which one seems to have useful information on the search results page,
say in a web answer, web result, video, etc. (you don\'t have to read the webpages).
